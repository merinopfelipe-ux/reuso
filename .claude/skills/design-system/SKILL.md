...
- Pie: SIEMPRE dos acciones (`Button variant="secondary"` + `Button variant="primary"|"danger"`), nunca un solo botón ni tres. **Única excepción** (2026-09-05): `sinPie` (booleano) omite el pie por completo, solo para contenido puramente informativo sin ninguna decisión que confirmar o cancelar (ej. un catálogo para explorar) — cerrar sigue disponible por la X, Escape o clic afuera. No uses `sinPie` para nada que sea una acción real, ahí sigue aplicando la regla de los 2 botones.
