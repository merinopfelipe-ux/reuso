...
// Generador de PDF del Acuerdo de Confidencialidad — extraído tal cual (texto
// legal intacto, sin reescrituras) desde la antigua ruta pública abierta
// /api/legal/firma, para reutilizarlo en el nuevo flujo cerrado por
// invitación (/legal/firma/[token]). El contenido legal en sí NO se toca,
// solo se movió de archivo.

export interface DatosFirmante {
  nombre: string
  tipoIdentidad?: string // si no se recolecta el tipo (flujo por invitación), se omite del PDF en vez de inventarlo
...
export function generarPdfConfidencialidad(data: DatosFirmante, fecha: string, ip: string, userAgent: string, verificationCode: string): Buffer {
  ...
  doc.setFontSize(9)
  const esRepresentanteEmpresa = Boolean(data.razonSocial && data.nit)
  const campos: [string, string][] = [
    ...(esRepresentanteEmpresa ? [
      ['Razón social', data.razonSocial!],
      ['NIT', data.nit!],
      ['Representante', data.nombre],
      ['Cargo del representante', data.cargoRepresentante ?? 'No informado'],
    ] as [string, string][] : [
