...
import { checkLimiteInformes } from '@/lib/plan-limits'
...
export async function POST(req: NextRequest) {
  ...
  // ── Rate limit ────────────────────────────────────────────────
  const allowed = await rateLimit(`informes_generar:${user.id}`, 5, 60 * 60 * 1000)
  if (!allowed) {
  ...
  const { data: empresa } = await adminClient
    .from('empresas')
    .select('nombre, logo_url, logo_propuesta_url, plan')
    .eq('id', empresaId)
    .single()
  empresaNombre = empresa?.nombre ?? null
  // logo_propuesta_url tiene prioridad: es donde queda el PNG derivado del
  // logo SVG subido desde /admin/empresas (jsPDF no soporta SVG).
  empresaLogoUrl = empresa?.logo_propuesta_url ?? empresa?.logo_url ?? null
  empresaPlan = (empresa?.plan ?? 'free') as Plan

  // Verificar límite de informes
  const errorLimite = await checkLimiteInformes(empresaId, empresaPlan)
  if (errorLimite) {
  ...
  const fileName = `informe_${codigoUUID}.pdf`
  const storagePath = `informes/${carpeta}/${fileName}`
  ...
  const { data: informeRow, error: insertError } = await adminClient
    .from('informes')
    .insert({
      tipo: 'informe',
      user_id: user.id,
  ...
  if (insertError || !informeRow) {
    ...
    if (removeError) {
      console.error('[informes/generar] PDF huérfano en storage - limpieza fallida:', storagePath, removeError)
    }
  ...
  await adminClient.from('logs_auditoria').insert({
    ...
    detalle_json: { informe_id: informeRow.id, co2_total: co2Total, empresa_id: empresaId, es_empresa: esDocumentoEmpresa },
  ...
  return NextResponse.json({
    ...
    codigo_verificacion: informeRow.codigo_verificacion,
    hash_integridad: hashIntegridad,
    pdf_url: pdfUrl,
    co2_total: co2Total,
  })
