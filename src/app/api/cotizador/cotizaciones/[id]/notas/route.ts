...
// Hilo de comentarios internos de una cotización, visible SOLO para el
// vendedor (empresa_admin/empleado dueños) y super_admin. El autor siempre
// se captura server-side, nunca del body.
//
// El autor se resuelve con una consulta aparte a profiles (nunca con el
// embed implícito `profiles:user_id(...)`): crm_cotizaciones_notas.user_id
...
// apellido y apodo completos para que el frontend pueda aplicar
// displayName() (apodo si es real, si no el primer nombre) — nunca el
// nombre completo crudo en temas de trazabilidad.
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: notas, error } = await adminClient
    .from('crm_cotizaciones_notas')
    .select('id, nota, created_at, user_id')
    .eq('cotizacion_id', params.id)
    .order('created_at', { ascending: true })

  if (error) {
    console.error('[GET /api/cotizador/cotizaciones/[id]/notas]', error)
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (error || !nota) {
    console.error('[POST /api/cotizador/cotizaciones/[id]/notas]', error)
