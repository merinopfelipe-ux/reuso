...
export function EquipoClient({ miembros, invitaciones: invitacionesIniciales, empresaId }: Props) {
  ...
  const handleInvitar = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/empresa/invitar', {
        ...
        body: JSON.stringify({ email, rol_asignado: rolInvitado, empresa_id: empresaId }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? 'Error al enviar la invitación.')
      setExito(`Invitación creada para ${email}`)
      setInvitaciones((prev) => [data.invitacion, ...prev])
      const link = `${window.location.origin}/invitacion/${data.rawToken as string}`
      setLinkInvitacion(link)
      setEmail('')
      // No cerrar modal — mostrar el link copiable
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Error inesperado.')
    } finally {
      setEnviando(false)
    }
  }, [email, rolInvitado, empresaId])
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{
                    ...
                    background: BG_LIGHT, display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 15, fontWeight: 700, color: BRAND,
                  }}>
                    {(m.nombre ?? '?')[0].toUpperCase()}
                  </div>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 600, color: TEXT_DARK, margin: 0 }}>{m.nombre}</p>
                    <p style={{ fontSize: 12, color: TEXT_MED, margin: 0 }}>Desde {formatFecha(m.created_at)}</p>
                  </div>
                </div>
                ...
                <span style={{
                  ...
                  padding: '3px 10px',
                  background: `${ROL_COLORS[m.rol] ?? '#4D7C79'}18`,
                  color: ROL_COLORS[m.rol] ?? TEXT_MED,
                }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: TEXT_MED, margin: 0 }}>
                  Expiran a los 7 días del envío
                </p>
              </div>
            </div>
          </div>
            ...
            {invitacionesPendientes.map((inv, idx) => {
              ...
              return (
                <div
                  ...
                  style={{
                    padding: '14px 20px',
                    borderBottom: idx < invitacionesPendientes.length - 1 ? `1px solid ${BORDER}` : 'none',
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    background: idx % 2 === 0 ? 'transparent' : `rgba(0,130,124,0.02)`,
                  }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      <p style={{ fontSize: 14, fontWeight: 600, color: TEXT_DARK, margin: 0 }}>{inv.email}</p>
                      <p style={{ fontSize: 12, color: TEXT_MED, margin: 0 }}>
                        {ROL_LABELS[inv.rol_asignado] ?? inv.rol_asignado} · Enviada {formatFecha(inv.created_at)}
                      </p>
                    </div>
                  </div>
      ...
      {modalOpen && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input
                ...
                style={{
                  ...
                  border: `1px solid ${BORDER}`, background: 'var(--bg-input)',
                  color: TEXT_DARK, fontSize: 14, outline: 'none', boxSizing: 'border-box',
                }}
              />
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <select
                ...
                style={{
                  width: '100%', padding: '10px 14px', borderRadius: 8,
                  border: `1px solid ${BORDER}`, background: 'var(--bg-input)',
                  color: TEXT_DARK, fontSize: 14, outline: 'none',
                }}
              >
            ...
            {/* Link copiable — aparece tras generar la invitación */}
            {linkInvitacion && (
              <div style={{
                background: 'rgba(0,130,124,0.08)',
                border: '1.5px solid rgba(0,130,124,0.25)',
                borderRadius: 10,
                padding: '12px 14px',
                marginBottom: 20,
              }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <p style={{
                    flex: 1, fontSize: 12, color: TEXT_MED, margin: 0,
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: copiado ? BRAND : 'transparent',
                      color: copiado ? '#fff' : BRAND,
            ...
            {!linkInvitacion ? (
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  ...
                  style={{
                    ...
                    background: email && !enviando ? BRAND : BG_LIGHT,
                    color: email && !enviando ? '#fff' : TEXT_MED,
                    fontSize: 14, fontWeight: 600,
                    cursor: email && !enviando ? 'pointer' : 'not-allowed',
              ...
              <button
                onClick={() => { setModalOpen(false); setLinkInvitacion(null); setError(null); setEmail('') }}
                style={{
