-- 086: Ocultar un ítem de la cotización sin eliminarlo — no cuenta en los
-- totales ni se muestra al cliente, pero sigue existiendo para el vendedor
-- (puede des-ocultarlo cuando quiera). Distinto de borrar (DELETE), que es
-- permanente.
...
BEGIN
  ...
  UPDATE crm_cotizaciones c SET
    subtotal              = v_subtotal,
    co2_evitado_total_kg  = COALESCE((SELECT SUM(m.co2_evitado_kg) FROM crm_muebles_cotizados m WHERE m.cotizacion_id = p_cotizacion_id AND m.oculto = false), 0),
    agua_evitada_total_l  = COALESCE((SELECT SUM(m.agua_evitada_l) FROM crm_muebles_cotizados m WHERE m.cotizacion_id = p_cotizacion_id AND m.oculto = false), 0),
    total                 = v_total,
    updated_at            = NOW()
